...
TESTS=Parser.cpp parser_test6b.cpp
...
clean:
	rm -rf *o $(EXECUTABLE) test_executable.out
	rm -rf coverage
	rm -f ./.depend
...
test_executable: $(GTESTDIR)/libgtest.a $(TESTS) depend
	$(CC) -isystem $(GTESTDIR) -pthread -ggdb $(TESTS) $(GTESTDIR)/gtest/gtest_main.cc $(GTESTDIR)/libgtest.a -o test_executable.out

test: test_executable
	./test_executable.out --gtest_print_time=0
	
coverage: test_executable
	rm -rf coverage
	mkdir coverage
	kcov --include-pattern=IntVec.cpp coverage/ ./test_executable
